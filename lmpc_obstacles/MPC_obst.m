function [x_mpc, u_mpc, avg_exec_time] = MPC_obst(x1)
...
Qd = (0.1^2/0.2^2)*(1/data.T^2)*diag([0, 0, 0, 0, 100, 100, 1e03, 0.01, 0.01, 100, 0, 0, 0]);
...
f1 = figure(1);
f1.Position = [100    100    1.25e03    0.5e03];
...
while x1(11) < data.L_track % Exit condition
	...
	d_r = data.c_o*(d_i-d_o)/2*sin(data.n_o*2*pi*x1(11)/data.L_track) + (d_i+d_o)/2;
